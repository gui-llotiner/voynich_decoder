...
The formula is: 		f(r)=C(r+ρ)−α
Where:
is the frequency of the word at rank r.
...
The formula for Shannon Entropy (H) is:	H(X)=−∑i=1n​p(xi​)logb​p(xi​)
Where:
...
This method calculates how predictable the next word is given the current word - functional words show low conditional entropy because they appear in fixed grammatical contexts, while content words show higher entropy due to semantic variability. Words falling below the language's average conditional entropy are classified as structural, while those above are classified as content words.
	On the table below, we can see the most frequent words in each language, in yellow, are the high frequency + high entropy (content word), in red, high frequency + low entropy (functional word), and, in blue,  words that are high frequency + low entropy and are, in fact, functional words. The first thing we can observe is that all languages maintain the same pattern, with slight variation for agglutinative languages such as Hungarian and Finnish. However, it is notable that the majority follow normal patterns dominated by functional words or topical words (culture, information etc.).



In Voynichese, from the 20 most common words, only 4 of them have low entropy, being "or", "s", "qokain", "qokedy" the words. In other languages we have a totally different scenario - almost all of them have more than 15 confirmed functional words in the top 20. At the same time, Voynich shows an unusual pattern where most words ranked 30-50 exhibit low entropy values, a distribution not observed in any comparison language. 
...
Our testing of these grammatical rules on the Voynich manuscript reveals they function as strict phonotactic constraints that govern character combinations and sequences. The empirical analysis shows a compliance rate of 56.9% of the unique words tested, demonstrating that Voynichese operates under systematic restrictions that limit which character sequences are linguistically valid, pointing to a non-natural language scope. This testing result explains the enhanced predictability patterns observed in our character transition entropy analysis, where knowing one character provides unusually strong constraints on what characters can follow. The grammar validation testing provides concrete evidence for the phonotactic restrictions that create the distinctive statistical signature of Voynichese.


Word Embeddings Application
Word embeddings represent words as dense vectors in a continuous vector space, capturing semantic and syntactic relationships based on contextual usage patterns. This computational technique allows us to analyze how words co-occur and relate to each other within the text, providing insights into the underlying linguistic organization. Applying embeddings analysis to Voynichese offers another quantitative approach to investigating its structural properties, complementing our entropy and grammar validation findings.
Word2Vec embeddings were trained using a sliding context window approach (window size=5, vector dimensions=100). The model learns vector representations where contextually similar words cluster closer together in the vector space, measured by cosine similarity. To validate the approach, we applied identical parameters to our comparison languages and verified that known linguistic relationships (synonyms, grammatical categories) produced expected clustering patterns.
For functional word detection, we calculated each word's similarity variance across its contextual neighbors. Functional words typically show low variance because they appear in predictable grammatical contexts, while content words exhibit higher variance due to their diverse semantic usage patterns.
Functional Words: The functional score represents a composite metric designed to quantify the likelihood that a word serves a grammatical rather than semantic role within the text. This score combines three normalized linguistic indicators: frequency percentile, entropy, and inverted similarity variance (reflecting the consistency of the word's usage patterns across different contexts). By averaging these three components, the functional score produces values between 0 and 1, where higher scores indicate words that behave like grammatical function words - appearing frequently, in predictable positions, with consistent usage patterns - while lower scores suggest content words with more variable semantic applications.


As we see above, we do have a high amount of words that act like highly functional words (some sections have almost 260), although they are not even on the most used words, as expected. Those words are used in different contexts leading to a low entropy and a low similarity variance. 
...
Part-of-Speech (POS)
To uncover potential grammatical roles without prior linguistic knowledge, we aimed to induce part-of-speech tags by clustering data into 10 categories. Using Pointwise Mutual Information (PMI) to vectorize the data, we applied k-means clustering to identify patterns. However, the results showed that most words grouped into a single cluster, cluster 7, revealing little about the language’s syntax. This outcome, while unexpected, aligns with the predominance of content words in natural language data.
